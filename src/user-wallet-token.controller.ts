...
export async function handleCreateUnsignedToken(
    ...
    try {
        const { userPublicKey, name, symbol, uri, supply, decimals } = req.body;
        console.log("📥 Create unsigned token request:", req.body);

        // Validate required fields
        if (!userPublicKey || !name || !symbol || !uri || !supply || !decimals) {
            return res.status(400).json({ 
                error: "Missing required fields: userPublicKey, name, symbol, uri, supply, or decimals." 
            });
        }
        ...
        if (validationResult.warnings.length > 0) {
            ...
            console.log("⚠️  Continuing with transaction creation despite metadata warnings...");
        } else {
        ...
        const result = await createUnsignedTokenTransaction(userPublicKey, tokenDetails);
...
export async function handleSubmitSignedTransaction(
    ...
    try {
        ...
        console.log("📥 Submit signed transaction request received");
